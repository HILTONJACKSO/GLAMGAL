          ...
          {products.slice(0, 4).map((product, index) => {
            ...
            return (
              ...
              >
                ...
                {/* Price & Action Footer */}
                <div className="pt-4 mt-4 border-t border-deep-charcoal flex items-center justify-between">
                  <div className="flex items-baseline space-x-2 font-display text-xs">
                    <span className="font-bold text-warm-white">
                      ${parseFloat(defaultVariant?.price.amount || product.priceRange.minVariantPrice.amount).toFixed(2)}
                    </span>
                    {defaultVariant?.compareAtPrice && (
                      <span className="text-warm-taupe line-through text-[10px]">
                        ${parseFloat(defaultVariant.compareAtPrice.amount).toFixed(2)}
                      </span>
                    )}
                  </div>
                  ...
                  {!isSoldOut && (
                    <button
                      ...
                      disabled={isLoading}
                      className="bg-warm-white hover:bg-[#B89275] text-obsidian hover:text-white px-3.5 py-1.5 rounded-full font-display text-[10px] tracking-wider uppercase transition-all flex items-center space-x-1.5 shadow-sm active:scale-95"
                    >
